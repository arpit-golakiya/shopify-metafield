...
const Index = (props) => {
    ...
    const { loading, error, data } = getBlogs(queryValue, status);

    const resourceName = {singular: 'blog', plural: 'blogs'}
    ...
    useEffect(()=>{
        ...
        console.log('getblogsResources', items);
        setProducts(items);
    },[data])
    ...
    return (
        <Page title="Blogs" subtitle="The Product metafield owner type." fullWidth>
            <Card>
              ...
              >
                {blogs.length>0 && blogs.map(({id,title,handle,url}, index) => {
                  ...
                  >
                    ...
                    <IndexTable.Cell>
                        <Link href={`/metafields/blog/`+ Encode(id)}>
                          <a className="area-settings-nav__action">
                            <div className="area-settings-nav__media">
                              <span className="menu-icons menu-settings">
