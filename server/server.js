...
app.prepare().then(async () => {
  ...
  server.use(
    createShopifyAuth({
      async afterAuth(ctx) {
        ...
        User.create({ shop: shop }, function (err, _user) {
        ...
        log.info('createShopifyAuth afterAuth ', { shop, accessToken, scope });
  ...
  const handleRequest = async (ctx) => {
    const {originalUrl, url, method, header: { host }} = ctx.request
    log.info('before handle ', ctx.query);
    await handle(ctx.req, ctx.res);
    log.info('after handle ', ctx.query);
  ...
  router.get("/", async (ctx) => {
    ...
    log.info('ACTIVE_SHOPIFY_SHOPS[shop] ', ACTIVE_SHOPIFY_SHOPS);
